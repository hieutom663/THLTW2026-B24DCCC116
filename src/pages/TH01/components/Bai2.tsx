...
const Bai2 = () => {
	...
	const handleSubmit = () => {
		form.validateFields().then((values) => {
			if (editingMonHoc) {
				const updated = monHocList.map((item) => (item.id === editingMonHoc.id ? { ...item, ...values } : item));
	...
	const handleSubmitTienDo = () => {
		tienDoForm.validateFields().then((values) => {
			...
			if (editingTienDo) {
				const updated = tienDoList.map((item) => (item.id === editingTienDo.id ? newData : item));
	...
	const danhMuc = [
		...
		},
		{
			...
			render: (_: any, record: any) => (
				<>
					<Button danger onClick={() => handleDeleteMonHoc(record.id)}>
						Xóa
					</Button>
					<Button type='primary' onClick={() => handleEditMonHoc(record)}>
						Sửa
					</Button>
				</>
			),
	...
	const handleSubmitMucTieu = () => {
		mucTieuForm.validateFields().then((values) => {
			...
			if (editingMucTieu) {
				const updated = mucTieuList.map((item) => (item.id === editingMucTieu.id ? newData : item));
	...
	const tienDo = [
		...
		},
		{
			...
			render: (_: any, record: any) => (
				<>
					<Button danger onClick={() => handleDeleteTienDo(record.id)}>
						Xóa
					</Button>
					<Button type='primary' onClick={() => handleEditTienDo(record)}>
						Sửa
					</Button>
				</>
			),
	...
	const mucTieu = [
...

		{
			...
			render: (_: any, record: any) => (
				<>
					<Button danger onClick={() => handleDeleteMucTieu(record.id)}>
						Xóa
					</Button>
					<Button type='primary' onClick={() => handleEditMucTieu(record)}>
						Sửa
					</Button>
				</>
			),
		...
		<>
			...
			>
				<Form form={form} layout='vertical'>
					...
					<Form.Item name='thoiLuongMonHoc' label='Thời lượng' rules={[{ required: true }]}>
