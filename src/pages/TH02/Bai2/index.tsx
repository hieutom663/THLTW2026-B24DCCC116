...
const Bai2_NganHangCauHoi: React.FC = () => {
  ...
  useEffect(() => {
    ...
    else datDanhSachKhoi([{ id: 'K1', tenKhoi: 'Tổng quan' }, { id: 'K2', tenKhoi: 'Chuyên sâu' }]);
  ...
  const luuDuLieu = (key: string, data: any, setter: any) => { setter(data); localStorage.setItem(key, JSON.stringify(data)); };
    ...
    <Card title="Bài 2: Hệ Thống Ngân Hàng Câu Hỏi & Sinh Đề Thi" style={{ margin: 24, borderRadius: 8 }}>
      <Tabs defaultActiveKey="1">
        <TabPane tab="1. Khối Kiến Thức & Môn Học" key="1">
          <QuanLyDanhMuc danhSachKhoi={danhSachKhoi} danhSachMon={danhSachMon} luuDuLieu={luuDuLieu} datDanhSachMon={datDanhSachMon} />
        </TabPane>
        
        <TabPane tab="2. Quản Lý Câu Hỏi" key="2">
          <QuanLyCauHoi danhSachCauHoi={danhSachCauHoi} danhSachMon={danhSachMon} danhSachKhoi={danhSachKhoi} luuDuLieu={luuDuLieu} datDanhSachCauHoi={datDanhSachCauHoi} />
        </TabPane>
        
        <TabPane tab="3. Sinh Đề Thi Tự Động" key="3">
          <QuanLyDeThi danhSachCauHoi={danhSachCauHoi} danhSachMon={danhSachMon} danhSachKhoi={danhSachKhoi} danhSachCauTruc={danhSachCauTruc} danhSachDeThi={danhSachDeThi} luuDuLieu={luuDuLieu} datDanhSachCauTruc={datDanhSachCauTruc} datDanhSachDeThi={datDanhSachDeThi} />
        </TabPane>
      </Tabs>
    </Card>
  );
